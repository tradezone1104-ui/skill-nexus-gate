      ...
      <div className="container mx-auto px-4 py-12">
        ...
        {purchases.length === 0 ? (
          <div className="bg-card rounded-xl border border-border p-12 text-center space-y-4">
            <Receipt className="h-16 w-16 text-muted-foreground mx-auto" />
            <h2 className="font-display font-semibold text-xl text-foreground">No purchases yet</h2>
            <p className="text-muted-foreground">Courses you buy will appear here.</p>
            <Link to="/courses">
              ...
              return (
                <div key={p.id} className="bg-card rounded-xl border border-border p-4 flex gap-4 items-center">
                  <Link to={`/course/${course.id}`} className="shrink-0">
                    <img src={course.thumbnail} alt={course.title} className="w-28 h-18 object-cover rounded-lg" />
                  </Link>
                  <div className="flex-1 min-w-0">
                    <Link to={`/course/${course.id}`}>
                      <h3 className="font-semibold text-foreground line-clamp-1 hover:text-primary transition-colors">{course.title}</h3>
                    </Link>
